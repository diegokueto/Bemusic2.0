<div class="box box-info padding-1">
    <div class="box-body">
        
        <div class="form-group">
            {{ Form::label('Nombre') }}
            {{ Form::text('Nombre', $instrumento->Nombre, ['class' => 'form-control' . ($errors->has('Nombre') ? ' is-invalid' : ''), 'placeholder' => 'Nombre']) }}
            {!! $errors->first('Nombre', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('Familia') }}
            {{ Form::text('Familia', $instrumento->Familia, ['class' => 'form-control' . ($errors->has('Familia') ? ' is-invalid' : ''), 'placeholder' => 'Familia']) }}
            {!! $errors->first('Familia', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('Material') }}
            {{ Form::text('Material', $instrumento->Material, ['class' => 'form-control' . ($errors->has('Material') ? ' is-invalid' : ''), 'placeholder' => 'Material']) }}
            {!! $errors->first('Material', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('País') }}
            {{ Form::text('País', $instrumento->País, ['class' => 'form-control' . ($errors->has('País') ? ' is-invalid' : ''), 'placeholder' => 'País']) }}
            {!! $errors->first('País', '<div class="invalid-feedback">:message</div>') !!}
        </div>

    </div>
    <div class="box-footer mt20">
        <button type="submit" class="btn btn-primary">Submit</button>
    </div>
</div>